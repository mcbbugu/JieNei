<template>
  <view class="dashboard">
    ...
    <!-- 主要内容区域 -->
    <view class="main-content" :style="{ paddingTop: navBarHeight + 'px', paddingBottom: (80 + safeAreaBottom) + 'px' }">
      <!-- 场馆热度标签 - 水平一条线布局 -->
      <view v-if="!loading" class="venue-section">
        <scroll-view scroll-x class="venue-scroll" :show-scrollbar="false">
          <view class="venue-list">
            ...
            <!-- 场馆列表 -->
            <view 
              v-for="([venueName, count], idx) in venueStats" 
              :key="venueName"
              class="venue-item"
              :class="{ active: activeFilter === `Venue:${venueName}` }"
              @click="setVenueFilter(venueName)"
            >
            ...
            <view class="card-top-row">
              <view class="left-section">
                <view class="intent-badge" :class="getIntentBgClass(player.intent)">
                  ...
                  <text class="intent-text" :style="{ color: getIntentColor(player.intent) }">{{ getIntentLabel(player.intent) }}</text>
                </view>
      ...
      >
        <view class="nav-icon" :class="{ active: currentTab === index }">
          <YuIcon 
            :name="item.icon" 
            :size="48" 
            :color="currentTab === index ? '#ccf381' : '#9ca3af'"
          />
        </view>
